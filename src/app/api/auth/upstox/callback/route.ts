...
function redirect(path: string) {
    const res = NextResponse.redirect(new URL(path, serverEnv.nextAuthUrl));
...
export async function GET(req: NextRequest) {
    try {
        ...
        if (error) {
            ...
            return redirect("/dashboard?upstox_error=access_denied");
        }
        ...
        if (!storedState || !returnedState || storedState !== returnedState) {
            ...
            return redirect("/dashboard?upstox_error=invalid_state");
        }

        // Validate authorization code
        if (!code) {
            return redirect("/dashboard?upstox_error=no_code");
        }
        ...
        return redirect("/dashboard?upstox_connected=true");
    } catch (error) {
        ...
        return redirect("/dashboard?upstox_error=connection_failed");
    }
}
